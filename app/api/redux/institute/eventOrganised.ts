...
interface InstituteEventOrganised {
  id: string;
  title: string;
  department: string;
  amount: number;
...
interface State {
  instituteEventOrganisedData : InstituteEventOrganised[];
  loading: boolean;
  error: string | null;
}

const initialState: State = {
  instituteEventOrganisedData : [],
  loading: false,
  error: null,
};
...
const instituteEventOrganisedSlice = createSlice({
  name: "consultancy",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchEventGrants.fulfilled, (state, action: PayloadAction<InstituteEventOrganised[]>) => {
      state.loading = false;
      state.instituteEventOrganisedData = action.payload;
    });
...
export default instituteEventOrganisedSlice.reducer;
