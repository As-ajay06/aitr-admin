...
interface TechnicalNonTechnical {
  id: string;
  title: string;
  department: string;
  amount: number;
...
interface State {
  technicalNonTechnicalData : TechnicalNonTechnical[];
  loading: boolean;
  error: string | null;
}

const initialState: State = {
  technicalNonTechnicalData : [],
  loading: false,
  error: null,
};
...
const technicalNontechnical = createSlice({
  name: "technicalNontechnical",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchEventGrants.fulfilled, (state, action: PayloadAction<TechnicalNonTechnical[]>) => {
      state.loading = false;
      state.technicalNonTechnicalData = action.payload;
    });
...
export default technicalNontechnical.reducer;
