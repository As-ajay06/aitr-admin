...
interface State {
  booksAUthoredData: BooksAUthored[];
  loading: boolean;
  error: string | null;
}
...
const booksChaptersAuthored = createSlice({
  name: "booksChaptersAuthored",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchEventGrants.fulfilled, (state, action: PayloadAction<BooksAUthored[]>) => {
      state.loading = false;
      state.booksAUthoredData = action.payload;
    });
...
export default booksChaptersAuthored.reducer;
