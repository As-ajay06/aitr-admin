...
interface State {
  reseachPaperGuidedData : ReseachPaperGuided[];
  loading: boolean;
  error: string | null;
}

const initialState: State = {
  reseachPaperGuidedData : [],
  loading: false,
  error: null,
};
...
const reseachPaperGuided = createSlice({
  name: "reseachPaperGuided",
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchEventGrants.fulfilled, (state, action: PayloadAction<ReseachPaperGuided[]>) => {
      state.loading = false;
      state.reseachPaperGuidedData = action.payload;
    });
